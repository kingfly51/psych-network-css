...
pdf(out("network.pdf"), width = 10, height = 8, useDingbats = FALSE)
graph <- plot(
  network, labels = cfg$short_labels, nodeNames = cfg$full_labels,
  groups = groups, color = palette, pie = predictability,
  layout = "spring", legend = length(cfg$communities) > 0,
  theme = "colorblind"
)
dev.off()
png(out("network.png"), width = 3000, height = 2400, res = 300)
plot(network, labels = cfg$short_labels, nodeNames = cfg$full_labels,
     groups = groups, color = palette, pie = predictability,
     layout = graph$layout, legend = length(cfg$communities) > 0,
     theme = "colorblind")
dev.off()
